...
User pastes image (⌘V)
    ...
    → thumbnails use pi-web file preview API for the selected machine/workspace
```
...
| "Paste panel is empty" | It only shows images pasted during the current browser session |
...
- `renderPanelGallery()` — session gallery in Paste panel
- `injectChatThumbnails()` — best-effort chat history thumbnails
...
- `knownImagesByWorkspace` — browser-session gallery, keyed by machine/project/workspace
...
| Gallery empty | Only session images are tracked | Paste again or inspect `.pi-paste/` manually |
