...
| Terminal tab shows `Save screenshot ...` after paste | pi-web preserves terminal command-run history even after the terminal process is closed | Current plugin closes finished terminals best-effort; a pi-web file-write API or ephemeral command-run option would be the clean fix |
| Old plugin behavior returns after update | Browser still has an older global paste listener in memory | Full-reload pi-web once; current versions register a global cleanup hook |
