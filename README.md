...
The server handles uploads, serves images back, and cleans up on request. Both communicate over `localhost:9876`. Images are always stored workspace-locally so remote agents can read the inserted `@.pi-paste/...` references.
...
- **Remote-safe** — images are referenced as `@.pi-paste/...`, never as local absolute paths
