  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        ...
        serverUrl: previewUrl(entry.path, runtime, entry.modifiedAt ?? entry.size ?? Date.now()),
        ts: entry.modifiedAt ?? Date.now(),
        size: entry.size,
      }));
  } catch (error) {
...
async function currentGalleryImages(runtime = currentRuntime) {
  const byPath = new Map();
  for (const image of knownImagesByWorkspace.get(workspaceKey(runtime)) ?? []) byPath.set(image.filePath, image);
  for (const image of await listWorkspacePasteImages(runtime)) byPath.set(image.filePath, image);
  return [...byPath.values()].sort((a, b) => String(b.ts ?? "").localeCompare(String(a.ts ?? "")));
}
...
async function renderPanelGallery() {
  const containers = querySelectorAllDeep(".screenshot-paste-panel-gallery");
  if (containers.length === 0) return;

  const runtime = currentRuntime;
  const runtimeKey = workspaceKey(runtime);
  const images = await currentGalleryImages(runtime);
  if (runtimeKey !== workspaceKey(currentRuntime)) return;
  if (images.length === 0) {
    containers.forEach((c) => {
      c.innerHTML = `<p class="muted">No screenshots found in <code>.pi-paste/</code>. Paste a screenshot (⌘V) to save one in this workspace.</p>`;
    });
    return;
  }
...
function startChatPoll() {
  ...
  window.__screenshotPasteChatPoll = setInterval(injectChatThumbnails, 1000);
}

function pasteRefsInUserMessages() {
  const refs = new Set();
  const candidates = querySelectorAllDeep("article, .msg, [data-message-id], chat-message");
