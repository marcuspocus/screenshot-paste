...
function updateWorkspaceFromContext(context) {
  const candidate = context?.workspace?.path
    ?? context?.state?.selectedWorkspace?.path
    ?? context?.state?.selectedWorkspace?.workspace?.path
    ?? null;
...
function requireWorkspacePath() {
  ...
  throw new Error("No workspace selected. Screenshot Paste must save images in workspace/.pi-paste/ so remote agents can read @file references.");
}
...
async function uploadImage(base64, filename) {
  ...
  const res = await fetch(`${SERVER_URL}/upload`, {
    ...
    body: JSON.stringify({ base64, filename, workspace }),
  });
  if (!res.ok) {
  ...
  try {
    for (const item of imageItems) {
      ...
      try {
        ...
        showToast(e?.message ?? "Failed to upload screenshot to server", 6000);
      }
    }
  } finally {
    isProcessing = false;
        ...
        actions: [
          {
            ...
            group: "Screenshot",
            enabled: (context) => {
              updateWorkspaceFromContext(context);
              return context.state.selectedSession !== undefined;
            },
            run: (context) => {
              updateWorkspaceFromContext(context);
              if (!serverAvailable) {
