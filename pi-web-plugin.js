...
function captureFilesApi(context, source) {
  ...
  bridgeFiles = {
    ...
    ...(typeof files.listFiles === "function" ? { listFiles: (path) => files.listFiles(path) } : {}),
...
// ── gallery: list .pi-web/paste via stable workspace files API ─────────────
async function listPasteImages(rt = getRuntime()) {
  if (!bridgeFiles || typeof bridgeFiles.listFiles !== "function") return [];
  try {
    const tree = await bridgeFiles.listFiles(PASTE_DIR);
    ...
    return {
      contributions: {
        actions: [
          {
            ...
            run: (context) => {
              updateRuntime(context);
              try {
                context.selectWorkspaceTool(`${runtimePluginId}:paste`);
                return;
              } catch {
        ...
        workspacePanels: [
          {
            id: "paste",
            routeAliases: ["screenshot-paste:paste"],
            title: "Paste",
            icon: svg`
